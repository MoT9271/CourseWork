...
public class Server {
    ...
    public static void main(String[] args) {
        ...
        try (ServerSocket serverSocket = new ServerSocket(portNumber)) {
            ...
            while (true) {
                ...
                ClientStreams clientStreams = new ClientStreams(in, out, clientSocket);
                ...
                new Thread(() -> handleClient(clientStreams)).start();
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public static void handleClient(ClientStreams clientStreams) {
        try {
            while (clientStreams.getSocket().isConnected() && clientStreams.isActive()) {
                ...
                else if("saveBuild".equalsIgnoreCase(action)){
                    handleSaveBuild(clientStreams);
                }
            }
    ...
    private static void handleSaveBuild(ClientStreams clientStreams) throws IOException {
        try {
            // Expecting UserInfo for saving build
            ...
            if (receivedObject instanceof UserInfo userInfo) {
                ...
                if (registeredUsers.containsKey(username)) {
                    // Update the user's hero information
    ...
    private static void handleLogin(ClientStreams clientStreams) throws IOException {
        try {
            ...
            if (receivedObject instanceof UserInfo userInfo) {
                ...
                if (registeredUsers.containsKey(username) && registeredUsers.get(username).password().equals(password)) {
    ...
    private static void loadUserData() {
        try (Reader reader = new FileReader(USER_DATA_FILE)) {
            MapType type = TypeFactory.defaultInstance().constructMapType(HashMap.class, String.class, UserInfo.class);
